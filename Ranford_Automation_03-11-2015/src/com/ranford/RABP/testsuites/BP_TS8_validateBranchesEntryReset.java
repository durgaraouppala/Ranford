...
public class BP_TS8_validateBranchesEntryReset {
	...
	Boolean bStatus;
	 ...
	 public void preCondition() 
	 {
		 ...
		 Excel.setExcelConnection("RABPTestData.xls");
		 Excel.selectSheet("newBranch");
		 Excel.setWritableWorkbook("RABPTestDataOuput.xls");//setting connection to writable wb
	...
	for(int iRowCount=1; iRowCount<Excel.getRowCount(); iRowCount++)
	 {
		 ...
		 if(bStatus)
		 {
			 ...
			 Excel.writeToCell(7, iRowCount, "PASS");
		 }
		 else
		 {
			 Reporter.log("branch Details entered not sucessfully",true);
			 Excel.writeToCell(7, iRowCount, "FAIL");
		 }	
		 Thread.sleep(2000);
		 AppFunctions.NewBranchesResetButton(driver);
		 bStatus=Util.verifyTextBoxValue(driver, Locators.NewBranchPage.txtBranchName);
		 if(bStatus)
		 {
			 Reporter.log("Reset is not successful. Branch name is not empty", true);
		 }
		 else
		 {
			 Reporter.log("Branch Name textbox is empty", true);
		 }
		}
	 }
